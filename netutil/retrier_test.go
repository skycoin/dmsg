...
func TestRetrier_Do(t *testing.T) {
	r := NewRetrier(logrus.New(), time.Millisecond*100, 3, 2)
	c := 0
	threshold := 2
	f := func() error {
		c++
	...
	t.Run("should return whitelisted errors if any instead of retry", func(t *testing.T) {
		bar := errors.New("bar")
		wR := NewRetrier(logrus.New(), 50*time.Millisecond, 1, 2).WithErrWhitelist(bar)
		barF := func() error {
			return bar
		}
	...
	t.Run("if times is 0, should retry until success", func(t *testing.T) {
		c = 0
		loopR := NewRetrier(logrus.New(), 50*time.Millisecond, 0, 1)
