...
func init() {
	...
	srvCmd.Flags().StringSliceVarP(&dmsgDisc, "dmsg-disc", "D", dmsgDisc, "dmsg discovery url(s)")
...
func server() {
	...
	dmsgC := dmsg.NewClient(pk, sk, disc.NewHTTP(dmsgDisc[0], &http.Client{}, log), dmsg.DefaultConfig())
	defer func() {
