...
// TransportMap represents a connection from dmsg.Client to remote dmsg.Client (via dmsg.Server intermediary)
...
type TransportMap struct {
	...
	ackWaiter ioutil.Uint16AckWaiterMap // awaits for associated ACK frames
...
// NewTransportMap creates a new dms_tp.
func NewTransportMap(conn net.Conn, log *logging.Logger, local, remote cipher.PubKey, id uint16, doneFunc func(id uint16)) *TransportMap {
	tp := &TransportMap{
		Conn:      conn,
		log:       log,
		id:        id,
		...
		doneFunc:  doneFunc,
		ackWaiter: ioutil.NewUint16AckWaiterMap(),
	}
...
func (tp *TransportMap) serve() (started bool) {
...
func (tp *TransportMap) close() (closed bool) {
	tp.doneOnce.Do(func() {
		closed = true
...
func (tp *TransportMap) Close() error {
	if tp.close() {
...
func (tp *TransportMap) IsClosed() bool {
	select {
	case <-tp.done:
		return true
	default:
...
func (tp *TransportMap) HandleFrame(f Frame) error {
	tp.inMx.Lock()
...
func (tp *TransportMap) WriteRequest() error {
...
func (tp *TransportMap) WriteAccept() (err error) {
	defer func() {
		if err != nil {
...
func (tp *TransportMap) ReadAccept(ctx context.Context) (err error) {
	defer func() {
		if err != nil {
...
func (tp *TransportMap) Read(p []byte) (n int, err error) {
	<-tp.serving
...
func (tp *TransportMap) Write(p []byte) (int, error) {
	<-tp.serving
